...
plot(kernelHR)
# create a mask raster called kernel - this isn't used later
ext(kernelHR)
kernels <- rast()
...
Next, note that the extents are all different for human access and elc_habitat-derived layers, so we need to resample landcover and human access layers to match extents.
...
Then we create and set values for landcover rasters based on landcover2 values of Habitat classification variables. Note below that we are using the command `ifel()` which is the `terra` equivalent of `ifelse()`.


```{r}

# group the categories into bigger categories and reclassify
# This will give you a spatial covariate for a dummy variable that you can plug into a logistic regression model 
...
# no map of openconifer - note that open conifer is implicitly being defined as the intercept
```
...
Note, the next two sets of commands give a way to export the processed rasters, all of them, to a new folder in similar format.  
```{r}
...
# these are now multi banded tifs where each band is a layer and then you name each layer with a band vector 
```
...
summary(top.biotic)
# reminder: null deviance is a model with no fit
#double check the VIF for each final model, just to be sure
## VIF is the variance information score - a measure of the variance between this model and the model that is a null model 
## Shows how much the variance  is blowing up bc of colinearity
## if they're all one they're fine, anything close to 5 is a problem
vif(top.biotic)
```
...
summary(top.env)
## Note that the deviance of this model is smaller than the biotic model 
vif(top.env)
## For this one, we see the closedConif and openConif are large, this is bc they are colinear with elevation
```
...
1) standard residuals - difference between model predications (probability) and observations (0 and 1)
2) studentized residuals - studentized normalizes the residuals - takes the raw residuals and divides them by the standard deviation, converts it to the scale of standard deviation so if anything is outside of 2 we know its outside of the range that we would observe by chance alone 
...
## fits probability of each point in the data frame for each point to be a wolf used point
...
## Manual residual plot - not very useful 
# in practice you don't look at residual plots with binomial distribution

# for binomials, you look at leverage
# Leverage plot - residuals vs cooks distance 
...
# Let's look at points that are outliers 
wolfkde3[16,]
# This point was at high elevation far from human access - it is a real wolf point even though our models says it's not supposed to be 
# this is only an influential point bc the data frame that it's based off of is from a wolf pack in low elevation

# This is what you use to go through and screen out errors in data entry 
...
Optimal cut point is determined by ratio to used available points (how many available points you generate) 
...
# Since this model wasn't as good, we expect a higher confusion area overlap
...
This shows a strong positive correlation between predicted probabilities of wolf use between the two models. This is good! We also see that there are gaps where there should be, so one model isn't predicting high quality habitat when one predicts low quality. There is also a lot of noise in this - the spread is pretty far. Naturally this leads us to think of things like averaging predictions across models, a point we will return to much later in ths semester when we consider ensemble modeling. 
...
#This shows us that one model, above somme threshold, doesn't perform as well as another model 
# the line on the model shows us that environmental model can't predict high quality biotic 
```
...
# our first manual confusion table 
...
# this will give you all of the outputs
...
![Figure 6.1. Calculations of different quantities from a Confusion Matrix. Note, Target represents the TRUTH, and Model represents the PREDICTIONS.  ](Figures/ConfusionMatrix.png)
...
perf3 <- performance(pred, "sens", x.measure = "cutoff") # takes your ROCR pred object and calculates sensitivity with some measure of cutoff
plot(perf3, xlab="Cutoff",ylab="Sensitivity")
```
...
plot(perf4,xlab="Cutoff/cut point",ylab= "Specificity")
```
...
cutoff
```
Thus the cutpoint for our data set that maximizes the overall classification success is 0.236. Note that this is VERY different from our naive starting value of 0.5. 
...
plot(acc.perf, main = "Overall Accuracy")
```
...
# this code splits your data into 5 partitions and validates the data witholding eac piece 
```
...
kxvFuzzFactor = 0.01
# take the formula from the top model 
# specify the data
# is the folds of validation - divide your dataset into 5 subsets and fit model to 4 and then predict to the 5th (simple random sample) 
# nbin is the number of categories we want to evaluate our predictions within - think like quartiles 
...
kfolds
# these graphs show you the frequency of ones by each habitat quality bins - both by each subset of the data and a mean with standard error graph
```
...
With the output of these, you want on your graph to see an increasing monotomic relationship between habitat quality and frequency of use 
...
# type = "response" means that you're looking for the probability

# 4. Make quantiles for the predictions - this calculates the 'bin's of the categories of habitat availability
q.pp = quantile(pred.prob,probs=seq(0,1,.1)) # use units .1 to get deciles 
...
# correlate the x vector against our counts of the observed frequencies of 1s
```
The result is .966, This suggests that in this random fold of the data, the model predicted habitat use well
...
## Last step of model validation: spatial prediction 
...
```{r}
par(mfrow = c(1,1))
# extract the coefficients from the top model 
...
This is raster calculator - we're going to calculate predicted probability for every cell in our raster layer. 
```{r}
# type in your raster math
...
plot(rast.top.biotic, col=colorRampPalette(c("yellow", "orange", "red"))(255), ext=kernels) # use the raster mask
plot(kernelHR, add=TRUE, col = NA)
plot(wolfyht, col='blue', pch = 16, add=TRUE)
```
The areas where no telemetry data was collected are areas being extrapolated to.

Interpolation and extrapolation difference:
...
knitr::purl(input = "README.Rmd", output = "lab6.R", documentation = 1)
```
